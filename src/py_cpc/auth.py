...
class DefaultAuth(AbstractAuth):
  ...
  async def async_get_access_and_id_token(self, client: AsyncClient) -> tuple[str, str]:
    if self._oauth_response:
      if self._oauth_response.token_expiration - datetime.datetime.now() > datetime.timedelta(minutes=1):
        return self._oauth_response.get_access_and_id_token()
      return await self._async_oauth_flow(client, "refresh_token", {"refresh_token": self._oauth_response.refresh_token})
    return await self._async_oauth_flow(client, "password", {"username": self._username, "password": self._password})
    
  async def _async_oauth_flow(self, client: AsyncClient, grant_type: str, parameters: dict[str, str]) -> tuple[str, str]:
    data = parameters | {
